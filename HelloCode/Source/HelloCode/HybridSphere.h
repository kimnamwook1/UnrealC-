...
public: 
	...
	void MyOnBeginOverlap_Implementation(AActor* OverlappedActor, AActor* OtherActor); // 기존 함수는 void로 납두고 implementation을 붙여서 오버라이드

	UFUNCTION(BlueprintNativeEvent)
	void MyOnEndOverlap(AActor* OverlappedActor, AActor* OtherActor);

	void MyOnEndOverlap_Implementation(AActor* OverlappedActor, AActor* OtherActor); // 기존 함수는 void로 납두고 implementation을 붙여서 오버라이드
};
