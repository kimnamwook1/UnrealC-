...
{
	if (SpawnTargets.Num() > 0 && ObstaclesToSpwan.Num() > 0)
	{
		short Spawner = FMath::Rand() % SpawnTargets.Num();
		short Obstacle = FMath::Rand() % ObstaclesToSpwan.Num();
		...
		AObstacle* newObs = GetWorld()->SpawnActor<AObstacle>(ObstaclesToSpwan[Obstacle], myTrans, SpawnInfo);
